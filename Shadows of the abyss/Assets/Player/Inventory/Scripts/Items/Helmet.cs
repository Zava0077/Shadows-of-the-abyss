...
public class Helmet : Armour
{
    void Awake()
    {
        ...
        gameObject.GetComponent<Slot>().values[0] += hpAdd != 0 ? hpAdd + maxHpSummand + rarity.maxHP : 0;
        gameObject.GetComponent<Slot>().values[29] += incripSlots + inscSummand + rarity.inscriptionAdded;
        gameObject.GetComponent<Slot>().values[10] += baseIgniteResistSummand != 0 ? baseIgniteResistSummand + igniteResistSummand + rarity.igniteResist : 0;
        gameObject.GetComponent<Slot>().values[9] += baseIceResistSummand != 0 ? baseIceResistSummand + iceResistSummand + rarity.iceResist : 0;
        gameObject.GetComponent<Slot>().values[11] += baseLightningResistSummand != 0 ? baseLightningResistSummand + lightningResistSummand + rarity.lightningResist : 0;
        gameObject.GetComponent<Slot>().values[12] += basePoisonResistSummand != 0 ? basePoisonResistSummand + poisonResistSummand + rarity.poisonResist : 0;
        gameObject.GetComponent<Slot>().values[14] += baseEvasionChance != 0 ? baseEvasionChance + evasionChanceSummand + rarity.evasionChance : 0;
        gameObject.GetComponent<Slot>().values[13] += baseVoidResistSummand != 0 ? baseVoidResistSummand + voidResistSummand + rarity.voidResist : 0;
